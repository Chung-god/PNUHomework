...
bool OutofRange(int row, int col) { 	
	if (row > -1 && row < MAX - 1 && col > -1 && col < MAX - 1) return true;
	return false;
}

//d 방향으로의 이동이 가능한지 판단하는 함수
bool isValid(char d,int row,int col) {
	switch (d)
	{
	case 'U':
		if (!visited[row - 1][col] && OutofRange(row - 1, col)) return true;
		return false;
		break;
	case 'D':
		if (!visited[row + 1][col] && OutofRange(row + 1, col)) return true;
		...
		break;
	case 'R':
		if (!visited[row][col + 1] && OutofRange(row, col + 1)) return true;
		return false;
		break;
	case 'L':
		if (!visited[row][col - 1] && OutofRange(row, col -1)) return true;
...
void backtracking(int cnt, int row, int col) {
	...
	for (int i = 1; i < 5;i++) {
		...
		else if (isValid(DRT[i], row, col)) {//가능한 방향
			//printf("Before cnt %d)row : %2d col : %2d\n", cnt, row, col);
